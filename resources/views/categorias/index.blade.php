...
@section('content')
    ...
    {{-- Los mensajes de éxito/error ahora se muestran con Toastify (ver vendor-scripts.blade.php) --}}

    {{-- Alerta si no hay categorías globales --}}
    @if ($categoriasGlobales->isEmpty())
        <div class="alert alert-warning">
            <i class="ri-alert-line me-2"></i>
            <strong>Atención:</strong> Primero debes crear
            <a href="{{ route('categorias-globales.index') }}" class="alert-link">Categorías Globales</a>
                    ...
                    <div class="d-flex flex-shrink-0 gap-2">
                        ...
                        @can('categorias.crear')
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                ...
                                {{ $categoriasGlobales->isEmpty() ? 'disabled' : '' }}>
                                ...
                                <tr data-id="{{ $categoria->id }}">
                                    <td>
                                        <h5><span class="badge badge-label bg-primary"><i
                                                    class="mdi mdi-circle-medium"></i>{{ $categoria->categoriaGlobal->nombre ?? '-' }}</span>
                                        </h5>
                                    </td>
                                    <td><strong>{{ $categoria->nombre }}</strong></td>
                                    <td>{{ Str::limit($categoria->descripcion, 40) ?? '-' }}</td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            <button type="button" class="btn btn-sm btn-info"
                                                onclick="verCategoria({{ $categoria->id }})" title="Ver">
                ...
                <form id="formCategoria" method="POST" action="{{ route('categorias.store') }}">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <select
                                class="form-select js-example-basic-single @error('categoria_global_id') is-invalid @enderror"
                                id="categoria_global_id" name="categoria_global_id" required>
                                <option value="">-- Seleccionar --</option>
                                @foreach ($categoriasGlobales as $global)
                                    <option value="{{ $global->id }}"
                                        {{ old('categoria_global_id') == $global->id ? 'selected' : '' }}>
                                        {{ $global->nombre }}
                                    </option>
                                @endforeach
                            </select>
...
@section('script')
    ...
    <script>
        window.CATEGORIAS_CONFIG = {
            ...
            categoriasGlobales: @json($categoriasGlobales),
            routes: {
    ...
    {{-- Script del módulo --}}
    <script src="{{ URL::asset('js/modules/categorias/index.js') }}"></script>
@endsection
