...
<head>
    ...
    <title>Ticket #{{ $venta->comprobante_completo }} | {{ $empresa->nombre_comercial }}</title>
...
<body>
    <div class="container">
        ...
        <div class="ticket-info">
            <div class="ticket-num">{{ strtoupper($venta->comprobante) }} {{ $venta->comprobante_completo }}</div>
            <div class="ticket-fecha">{{ $venta->fecha_emision->format('d/m/Y h:i A') }}</div>
        ...
        <div class="content-box">
            <div class="info-row">
                ...
                <span class="info-value">{{ $venta->nombre_cliente }}</span>
            </div>
            @if ($venta->cliente)
                <div class="info-row">
                    <span class="info-label">{{ $venta->cliente->tipo_documento }}:</span>
                    <span class="info-value">{{ $venta->cliente->numero_documento }}</span>
                </div>
            @endif
        </div>
                    ...
                    @foreach ($venta->detalles as $detalle)
                        <tr>
                            <td>
                                ...
                                <span class="item-detalle">S/ {{ number_format($detalle->precio_unitario, 2) }}</span>
                            </td>
        ...
        <div class="totals">
            ...
            <div class="total-row">
                <span class="info-label">IGV ({{ number_format($venta->igv_porcentaje, 0) }}%):</span>
                <span class="info-value">S/ {{ number_format($venta->igv_monto, 2) }}</span>
            </div>
            @if ($venta->descuento > 0)
                <div class="total-row" style="color: #f06548;">
                    <span class="info-label">Descuento:</span>
                    <span class="info-value">- S/ {{ number_format($venta->descuento, 2) }}</span>
                </div>
            @endif
            ...
            <div class="total-row" style="margin-top: 15px;">
                ...
                <span class="info-value text-uppercase">{{ $venta->metodo_pago }}</span>
            </div>

            @if ($venta->es_credito)
                <div class="total-row">
                    <span class="info-label">Saldo Pendiente:</span>
                    <span class="info-value text-danger">S/ {{ number_format($venta->saldo_pendiente, 2) }}</span>
                </div>
            @endif
        </div>
        ...
        <div class="footer">
            ...
            <p>Vuelva pronto a {{ $empresa->nombre_comercial }}</p>

            @if ($venta->comprobanteElectronico)
                <p style="font-size: 11px; margin-top: 10px;">
                    Representación impresa de la {{ $venta->comprobanteElectronico->tipo_comprobante_nombre }}
                    electrónica.
            ...
            <div class="d-flex gap-2 justify-content-center" style="margin-top: 20px;">
                ...
                <a href="https://api.whatsapp.com/send?text=Hola! Aquí tienes mi ticket de compra: {{ request()->fullUrl() }}"
