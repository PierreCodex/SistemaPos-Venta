                        ...
                        <div>
                            <h4 class="fs-22 fw-semibold ff-secondary mb-4">
                                <span class="counter-value"
                                    data-target="{{ $estadisticas['movimientos_hoy'] ?? 0 }}">0</span>
                            </h4>
                        </div>
                    ...
                    <div class="d-flex align-items-end justify-content-between mt-4">
                        <div>
                            <h4 class="fs-22 fw-semibold ff-secondary mb-4 text-success">
                                +<span class="counter-value" data-target="{{ $estadisticas['entradas_mes'] ?? 0 }}">0</span>
                            </h4>
                        </div>
                        <div class="avatar-sm flex-shrink-0">
                    ...
                    <div class="d-flex align-items-end justify-content-between mt-4">
                        <div>
                            <h4 class="fs-22 fw-semibold ff-secondary mb-4 text-danger">
                                -<span class="counter-value"
                                    data-target="{{ abs($estadisticas['salidas_mes'] ?? 0) }}">0</span>
                            </h4>
                        </div>
                        <div class="avatar-sm flex-shrink-0">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-end justify-content-between mt-4">
                        <div>
                            <h4 class="fs-22 fw-semibold ff-secondary mb-4">
                                <span class="counter-value"
                                    data-target="{{ $estadisticas['total_movimientos'] ?? 0 }}">0</span>
                            </h4>
                        </div>
                        <div class="avatar-sm flex-shrink-0">
                        ...
                        <tbody>
                            @foreach ($movimientos as $mov)
                                <tr>
                                    ...
                                    <td>
                                        <strong>{{ $mov->producto->nombre ?? 'N/A' }}</strong>
                                    ...
                                    <td class="text-end">
                                        @if ($mov->cantidad >= 0)
                                            <span
                                                class="text-success fw-semibold">+{{ number_format($mov->cantidad, 2) }}</span>
                                        @else
                                            <span
                                                class="text-danger fw-semibold">{{ number_format($mov->cantidad, 2) }}</span>
                                        @endif
                                    </td>
                                    <td class="text-end">{{ number_format($mov->stock_anterior, 2) }}</td>
                                    <td class="text-end fw-semibold">{{ number_format($mov->stock_resultante, 2) }}</td>
                                    <td>
                                        <small class="text-muted">{{ Str::limit($mov->observaciones, 30) ?? '-' }}</small>
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <span>{{ $mov->user->name ?? 'Sistema' }}</span>
                                        </div>
                                    </td>
                                    <td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#filtro_producto').select2({
                ...
                width: '100%',
                ajax: {
                    url: '{{ route('inventario.kardex.api.buscar-productos') }}',
                    dataType: 'json',
                    delay: 250,
                    data: params => ({
                        q: params.term
            ...
            $('#btnLimpiar').on('click', function() {
                ...
                $('#filtro_tipo').val('');
                $('#fecha_desde').val('{{ now()->subMonth()->format('Y-m-d') }}');
                $('#fecha_hasta').val('{{ now()->format('Y-m-d') }}');
