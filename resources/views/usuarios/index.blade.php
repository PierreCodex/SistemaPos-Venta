                    ...
                    <div class="d-flex align-items-end justify-content-between mt-4">
                        <div>
                            <h4 class="fs-22 fw-semibold ff-secondary mb-4" id="totalUsuarios">
                                <span class="counter-value" data-target="{{ $users->count() }}">{{ $users->count() }}</span>
                            </h4>
                        </div>
                    ...
                    <div class="d-flex align-items-end justify-content-between mt-4">
                        <div>
                            <h4 class="fs-22 fw-semibold ff-secondary mb-4">
                                {{ $users->filter(fn($u) => $u->hasRole('administrador'))->count() }}
                            </h4>
                        </div>
                        <div class="avatar-sm flex-shrink-0">
            ...
            <div class="card card-animate">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-end justify-content-between mt-4">
                        <div>
                            <h4 class="fs-22 fw-semibold ff-secondary mb-4">
                                {{ $users->filter(fn($u) => $u->hasRole('vendedor'))->count() }}
                            </h4>
                        </div>
                        <div class="avatar-sm flex-shrink-0">
                                    ...
                                    @foreach ($roles as $role)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="roles[]"
                                                value="{{ $role->name }}" id="roleCreate_{{ $role->id }}">
                                            <label class="form-check-label" for="roleCreate_{{ $role->id }}">
                                                {{ ucfirst($role->name) }}
                                            </label>
                                        </div>
                                    @endforeach
                            ...
                            <div class="col-12">
                                ...
                                <div class="d-flex flex-wrap gap-3 mb-2" id="rolesCheckboxesEdit">
                                    @foreach ($roles as $role)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="roles[]"
                                                value="{{ $role->name }}" id="roleEdit_{{ $role->id }}">
                                            <label class="form-check-label" for="roleEdit_{{ $role->id }}">
                                                {{ ucfirst($role->name) }}
                                            </label>
                                        </div>
                                    @endforeach
...
@section('script')
    ...
    <script>
        const ROUTES = {
            list: '{{ route('usuarios.api.list') }}',
            store: '{{ route('usuarios.api.store') }}',
            show: '{{ route('usuarios.api.show', ':id') }}',
            update: '{{ route('usuarios.api.update', ':id') }}',
            destroy: '{{ route('usuarios.api.destroy', ':id') }}'
        };

        const PERMISOS = {
            canEdit: {{ auth()->user()->can('usuarios.editar') ? 'true' : 'false' }},
            canDelete: {{ auth()->user()->can('usuarios.eliminar') ? 'true' : 'false' }}
        };
                        ...
                        <div class="card user-card border">
                            <div class="card-body text-center">
                                <div class="avatar-lg mx-auto mb-3">
                                    <img src="${user.avatar ? '/images/' + user.avatar : '{{ URL::asset('build/images/users/avatar-1.jpg') }}'}" 
